...
//                Log.d("observe", "$col $row")
    ...
    fun clearMarker(col: Int, row: Int) {
        // XXX Write me
        update(col, row, HexState.Unclaimed)
    }
...
//                Log.d("clearFinishedGame", "$col $row ${board[col][row]}")
    ...
    private fun lightPath(path: MutableList<Pair<Int, Int>>) {
        ...
        MainScope().launch {
            for (i in 0 until path.size) {
                ...
                val row = pair.second
                val col = pair.first
                update(col, row, circlefy(board[col][row]))
                delay(100)
            }
        }
    }
    ...
    private fun didWin(
        graph: Graph<Pair<Int, Int>>,
        startBorder: Pair<Int, Int>,
        endPairs: List<Pair<Int, Int>>
    ): Boolean {
